...
VALUES (
    ...
    '[
        ...
        {"id": "a2", "points": [[6, 4], [6, 7]], "direction": "down"},
        {"id": "a3", "points": [[5, 6], [2, 6]], "direction": "left"},
        {"id": "a4", "points": [[2, 5], [2, 3]], "direction": "up"}
    ]'::jsonb
) ON CONFLICT (level_number) DO UPDATE SET arrows = EXCLUDED.arrows;

-- Level 2: Gentle Tangle (L-turns)
INSERT INTO levels (level_number, title, difficulty, grid_size, arrows)
VALUES (
    2,
    'Corner Turns',
    ...
    8,
    '[
        {"id": "a1", "points": [[2, 7], [2, 2], [6, 2]], "direction": "right"},
        {"id": "a2", "points": [[7, 2], [7, 6], [4, 6]], "direction": "left"},
        {"id": "a3", "points": [[3, 5], [3, 3]], "direction": "up"},
        {"id": "a4", "points": [[5, 4], [5, 7]], "direction": "down"},
        {"id": "a5", "points": [[4, 7], [7, 7]], "direction": "right"}
    ]'::jsonb
) ON CONFLICT (level_number) DO UPDATE SET arrows = EXCLUDED.arrows;
